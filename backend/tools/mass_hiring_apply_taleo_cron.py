...
def main() -> None:
    ...
    logger.info("applying to %d TTEC jobs x %d round(s) = %d applications (workers=%d, pay-ordered, open-only)",
                len(ids), args.rounds, len(batch), workers)
    if not batch:
        logger.info("every TTEC job already reached interview/offer — nothing to apply")
        return
