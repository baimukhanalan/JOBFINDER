...
def _pick_proxy() -> dict | None:
    ...
    # residential/phone is OPT-IN (WORKDAY_RESIDENTIAL=1), NOT the default: the only live slots are KZ
    # phones — geo-mismatched for a US application AND slow (a US Workday page failed to render through
    # one). DIRECT (the fast French datacenter IP) renders fine + is the right default; prefer a US IP
    # via WORKDAY_BRIGHTDATA=1 or WORKDAY_PROXY=<us-url>.
    if (os.getenv("WORKDAY_RESIDENTIAL") or "").strip().lower() in ("1", "true", "yes", "on"):
        try:
