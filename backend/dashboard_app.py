...
def unfinished_index():
    ...
    for it in items:
        ...
        cap = ats in ("lever", "workable")
        rcls = "unf-reason cap" if (cap or it.get("blocked")) else "unf-reason"
        ...
        cards.append(
            f'<article class="unf-card" data-id="{esc(str(jid))}">'
            ...
            f'<button class="unf-go" data-id="{esc(str(jid))}" onclick="unfFinish(this)">Докрутить</button>'
            f'{open_link}'
    ...
    css = ("<style>"
           ...
           "font-weight:700;color:var(--ink);margin:0 0 14px}.unf-head b{color:var(--accent)}"
           ".unf-head .unf-links{font-size:13px;font-weight:600;margin-left:auto;display:flex;gap:14px}"
           ".unf-head a{color:var(--accent);text-decoration:none}.unf-head a:hover{text-decoration:underline}"
           ".unf-list{display:flex;flex-direction:column;gap:10px}"
           ".unf-card{background:var(--panel);border:1px solid var(--line);border-radius:var(--r-sm);padding:14px 16px}"
           ...
           ".unf-reason{margin-top:5px;font-size:13px;font-weight:600;color:var(--ink-soft)}"
           ".unf-reason.cap{color:var(--danger)}"
           ".unf-meta{margin-top:4px;font-size:11.5px;color:var(--ink-mute);font-family:var(--ff-mono)}"
           ".unf-actions{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:12px}"
           ".unf-go{background:#0b8043;color:#fff;border:none;border-radius:var(--r-full);padding:9px 18px;"
           "font-size:13px;font-weight:700;cursor:pointer;min-height:40px}.unf-go:hover{background:#0a7038}"
           ".unf-go:disabled{opacity:.5;cursor:default}"
           ".unf-open{font-size:13px;font-weight:600;color:var(--accent);text-decoration:none}"
           ".unf-open:hover{text-decoration:underline}"
           ...
           ".unf-done:hover{border-color:var(--danger);color:var(--danger)}"
           ".unf-res{font-size:12.5px;color:var(--ink-mute)}"
           ".unf-empty{color:var(--ink-mute);text-align:center;padding:48px 20px;font-size:15px;font-weight:600}"
    ...
    js = ("<script>"
          ...
          "res=btn.parentElement.querySelector('.unf-res');"
          ...
          "catch(e){btn.disabled=false;btn.textContent='Докрутить';"
          ...
          "if(j.ok&&card){card.remove();}else{btn.disabled=false;}}catch(e){btn.disabled=false;}}"
          "</script>")
