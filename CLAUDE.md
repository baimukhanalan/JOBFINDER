...
- **Concentrix** (`fetch_concentrix` = jdq feed + legacy Workday board, merged & deduped on `(source,source_id)`). **Root-caused 2026-09-21: the corporate Workday board `cnx.wd1/external_global` is a GLOBAL/PROFESSIONAL board** (1579 jobs, offshore-dominated — PH 342 / MY 193 / ES 185 / IN 91 …; US country facet only **~27**, almost all Director/Principal-Architect/Sr-Manager/Developer or on-site → exactly **1** remote-US entry CSR; `External_US` site is empty, no alternate `cnx` tenant/host exists, `jobs.concentrix.com` just embeds `external_global`). The real US work-at-home **frontline** (Licensed Health Insurance Rep, CS / Tech Support Rep, Insurance Agent, Inside Sales Rep, … ~$15–23/hr, seasonal) lives on Concentrix's own first-party feed **`jobs.concentrix.com/wp-json/jdq/v1/search`** (`country="USA"` → ~68 US rows; structured `remote_type` fully_remote|hybrid|on_site; hourly rate only in `long_description` prose → `_parse_hourly_wage`). That feed AGGREGATES BOTH apply channels: Workday reqs (`apply_url` on `cnx.wd1.myworkdayjobs.com`) AND **Talkpush** frontline campaigns (`apply_url` on `concentrix.crew.talkpush.com`). `_cnx_jdq_row` (PURE, unit-tested) keeps the fully-remote + `categorize()`-passing set and stores each row's NATIVE `apply_url` — so the Workday auto-apply cron (`workday_ids()` selects `apply_url ILIKE '%myworkdayjobs.com%'`) is UNAFFECTED by the Talkpush rows, and the ~7 frontline entry roles feed the human/collect board (a Talkpush auto-apply lane is NOT built — those are collect-only for now). Legacy Workday-board fetch is kept as a safety net so the proven Workday-apply lane's categorized req is never lost. **Yield 1 → ~8** (7 Talkpush entry + 1 Workday). `categorize()` is NOT over-dropping — every dropped external_global US row is genuinely senior/corporate/dev/on-site.
...
- **Wayfair** (added 2026-09-21; `fetch_wayfair` = `_fetch_smartrecruiters("wayfair","Wayfair")`, reuses `_smartrecruiters_row` VERBATIM) — a DIRECT-HIRE (NOT a BPO) remote "Virtual Sales & Service" employer on the SAME SmartRecruiters postings API `api.smartrecruiters.com/v1/companies/Wayfair/postings` (`Wayfair` slug is case-insensitive; the variants `WayfairLLC`/`WayfairCareers`/… are empty). Same keep-`location.country=="us"`+`.remote==True` filter + `categorize()` entry gate. **`categorize()` gained a NARROW `sales (and|&) service (consultant|rep|representative|associate|specialist|agent|advisor)` alternative in the `customer_support` regex** — Wayfair's flagship "Virtual Sales & Service Representative" / "Sales and Service Consultant" titles matched NO category before (they're not "customer service"), so the connector would have yielded 0 even when the board fills; the entry-noun suffix keeps a "Sales & Service Team Lead"/senior brush `_NOT_MASS`-vetoed. **HONEST LIVE YIELD (2026-09-21): the Wayfair SR board currently has `totalFound=1` TOTAL — a single ON-SITE "Spanish Bilingual Service Consultant" (Bryan TX, `remote=False`) → 0 remote-US entry rows RIGHT NOW.** Collect-first: the connector auto-captures Wayfair's remote Virtual Sales & Service roles the moment they open (Wayfair runs seasonal remote-CSR ramps), like the Transcom/Percepta ~1-row connectors. **Apply reuses the SR lane with ZERO cron change** (see the SR apply lane below). `auto_status` mirrors Sutherland (`needs_laptop` badge; the SR cron drives it full-auto regardless — the badge is cosmetic). Tests: `test_wayfair_*` + `test_sales_and_service_entry_roles_categorize` in `test_mass_hiring.py` (network-free).
...
- **BPOs on already-supported ATSes (added 2026-09-21; `fetch_transcom`/`fetch_percepta`, apply reuses the tenant's existing strategy after a per-tenant verify pass — collect-first today, NOT auto-picked by any live lane).** Both are US-tiny right now (Transcom ~1, Percepta ~1 genuinely-remote-US) but auto-capture any future US remote CSR ramp; the pure `_transcom_row`/`_percepta_row` helpers are network-free unit-tested.
  ...
  - **Percepta** (`fetch_percepta`/`_percepta_row`/`_percepta_loc`): Ford-affiliated BPO on **Taleo**. TWO careersections — the **US portal is 10300** (portal `128160131726`); 10400 is international (0 US). Modern FACETED Taleo → the whole board comes from ONE endpoint: `POST /careersection/rest/jobboard/searchjobs?lang=en&portal=<id>` (**NO careersection code in the path** — `/careersection/rest/…`, not `/careersection/10300/rest/…`, which 404s); GET `…/careersection/10300/jobsearch.ftl` first to set the session cookie; JSON body `{fieldData:{fields:{KEYWORD:"",LOCATION:""}},…,pageNo}` (`pagingData.totalCount`, pageSize 25). A requisition's `column` array = `[title, locations-json]`; `linkedColumn` indexes the title, `locationsColumns` the location column(s) whose value is a JSON array of Taleo codes (`"US-MI-Dearborn"` → `_percepta_loc` formats "Dearborn, MI, United States"). Most US CSR reqs are SITE-based (Melbourne FL / Dearborn MI / Mahwah NJ) so the remote-only rule keeps just the genuinely-remote ones. Apply = **Taleo → reuses `strategies/taleo.py`**, but the TTEC Taleo apply cron is scoped to `source='ttec'` so a `source='percepta'` row is never driven by the wrong (TTEC-tuned) `taleo_recon`; wiring it live needs a Percepta-tenant verify pass.
- **Molina Healthcare — Oracle Recruiting Cloud (`fetch_molina`/`_orc_row`/`_fetch_orc`, added 2026-09-21)** — the SAME public ORC REST board as Alorica: `GET hckd.fa.us2.oraclecloud.com/hcmRestApi/resources/latest/recruitingCEJobRequisitions?finder=findReqs;siteNumber=CX_1,limit=50,offset=N,sortBy=POSTING_DATES_DESC` (`careers.molinahealthcare.com` fronts it; the `molinahealthcare.wd1` Workday host is stale/parked). `_orc_row` (PURE, unit-tested, shared-shape with the inline Alorica logic): US from `PrimaryLocationCountry=="US"`, remote from the Title/`PrimaryLocation` (or a bare-country "United States" national posting), then `categorize()`. **HONEST live yield 2026-09-21: board 341, remote+US 133, remote-US ENTRY CSR ~3** (Pharmacy CSR / Pharmacy Customer Service Rep / Provider-Engagement Specialist; the rest is Director/RN/Medical-Director/clinical, correctly dropped). Apply **reuses `strategies/oracle_orc.py` (the Alorica lane)** BUT `orc_recon.orc_job_ids()` selects `source='alorica'` only + the ORC cron is INERT until `ORC_PHONE`, so Molina is **COLLECT-FIRST** (`auto_status='needs_laptop'`); wiring it live = widen `orc_job_ids` to `source IN ('alorica','molina')` + a per-tenant screener verify pass (Molina's ORC form differs from Alorica's). Tests: `test_orc_*` in `test_mass_hiring.py`.
- **Kaiser Permanente — Radancy TalentBrew → Taleo apply (`fetch_kaiser`/`_kaiser_row`, added 2026-09-21)** — collected off the Radancy front (like TTEC/UnitedHealth): `GET www.kaiserpermanentejobs.org/search-jobs/results?Keyword=remote&CurrentPage=N&RecordsPerPage=100&SearchType=5` returns a JSON envelope whose `results` is an HTML fragment (`a[data-job-id]`, `_BROWSER_UA`). Kaiser's `.job-location` cell is a COMMA LIST "City, ST, <workplace>, <schedule>…" where `<workplace>`∈{Remote,Onsite,Flexible} → `_kaiser_row` reads remote off that token (or the title), US forced True (US-only site). **HONEST live yield 2026-09-21: ~6 remote entry** (Contact Center Specialist I, mostly WA/GA; the bulk of tiles are Onsite clinical nurses, correctly dropped). Apply forwards to **Oracle Taleo `kp.taleo.net` → reuses `strategies/taleo.py`** after a per-tenant Basics-mapping verify (the TTEC Taleo cron is scoped `source='ttec'`, so a `source='kaiser'` row is never driven by the wrong tuning) → **COLLECT-FIRST** (`auto_status='needs_laptop'`), like Percepta. Tests: `test_kaiser_*` in `test_mass_hiring.py`.
- **Coverage expansion 2026-09-22 — Everise (BPO) + Oscar/Clover/Devoted (healthcare payers) + the FIRST Canada feeds (Concentrix CA + Sutherland CA).** All COLLECT-FIRST (`auto_status='needs_laptop'`); network-free `_row`/`_workday_row`/`_greenhouse_row`/`_smartrecruiters_row`/`_cnx_ca_row` decisions unit-tested. Two shared helpers gained OPT-IN params (existing callers byte-identical): `_workday_row`/`_fetch_workday` gained **`us_from_path`** (confirm US from a state in the `externalPath` slug when `locationsText` is a bare "N Locations"), and `_smartrecruiters_row`/`_fetch_smartrecruiters` gained **`country`** + **`force_eligible`** (for a non-US board slice).
  - **Everise** (`fetch_everise` → `_fetch_workday`): US remote-CSR BPO on Workday `weareeverise.wd1.myworkdayjobs.com/weareeverise/everiseCareers`. Everise is GLOBAL (US/PH/Guatemala/Malaysia) so NOT `us_confirmed`; US is read per-row via `us_from_path=True` (most remote reqs carry the state in the path, e.g. `…/Work-from-Home-Alabama/…`, even when `locationsText` is "29 Locations") + `title_remote=True` (WFH title). **STRONG live yield 2026-09-22: board ~40, ~17 US-remote, ~6 remote-US ENTRY CSR NOW** ("Healthcare Customer Service Representative - Work From Home", "Licensed Health Insurance Agent- Work from Home"). Apply = Workday create-account lane after a per-tenant verify.
  - **Oscar Health** (`fetch_oscar`) + **Clover Health** (`fetch_clover`) → `_fetch_greenhouse`/`_greenhouse_row` (NEW keyless GH boards-api reader `boards-api.greenhouse.io/v1/boards/<token>/jobs`): US Medicare-Advantage insurers posting remote member-services CSR alongside clinical/eng roles (`categorize()` drops the latter; a bare "Remote" is US since these are US-only payers). **HONEST live 2026-09-22: oscar ~1 (COB Verification Specialist) · clover ~1 (Provider Engagement)** — future-proof (seasonal member-services ramps). Custom GH form ⇒ collect-first (no wired apply). Boards: oscar `oscar`, clover `cloverhealth`.
  - **Devoted Health** (`fetch_devoted` → `_fetch_workday`): US-only MA insurer on Workday `devoted.wd1.myworkdayjobs.com/devoted/Devoted` (`us_confirmed=True`, `title_remote=True`). **HONEST live 2026-09-22: board ~76, ~3 remote ENTRY** (Digital/Pharmacy Care Coordinator; the bulk is senior/clinical, dropped). Future-proof. Apply = Workday lane after a per-tenant verify.
  - **CANADA — Concentrix Canada** (`fetch_concentrix_canada`/`_cnx_ca_row`): the SAME first-party jdq feed as US (`jobs.concentrix.com/wp-json/jdq/v1/search`, `country=Canada`), source `concentrix_ca`. `_cnx_ca_row` mirrors `_cnx_jdq_row` but builds a Canadian `location_raw` and **FORCES `us_eligible=True`** so `collect(us_only=True)` keeps it (the board has NO CA column — this flag means "keep on the North-America board", location carries the Canada signal). **STRONG live yield 2026-09-22: ~37 CA rows, ~10 remote ENTRY CSR** (fully_remote WFH, bilingual EN/FR). Reuses the Talkpush/Workday apply channels; collect-first.
  - **CANADA — Sutherland Canada** (`fetch_sutherland_canada` → `_fetch_smartrecruiters(..., country="ca", force_eligible=True)`, source `sutherland_ca`): the SAME SR postings API as US Sutherland, `location.country=="ca"`. **HONEST live 2026-09-22: ~2 CA-remote, ~1 CA-remote entry** (tiny, future-proof). The SR apply cron keys on the `jobs.smartrecruiters.com` apply_url HOST (not `source`), so these CA rows CAN be driven by the shared SR lane (cosmetic `needs_laptop`, like Wayfair) after a per-tenant verify.
  - **NOT ADDABLE / not added (recon 2026-09-22, honest):** **iQor** (careers.iqor.com 403 — Cloudflare/JS SPA, no JSON even via curl_cffi), **TELUS Digital / TELUS International** (careers 403, JS SPA), **WNS/ResultsCX/HGS/IBEX** (careers hosts DNS-fail or no discoverable ATS JSON). **Genpact** (Workday `genpact.wd108/External_Careers`, 2000 reqs) + **Startek** (Oracle ORC `fa-evuf-…oraclecloud.com`, 1804 reqs) ARE reachable but are OFFSHORE-dominated (India/PH) with ~0 US-remote-ENTRY today — NOT added to avoid heavy no-yield nightly paging. **Wellcare** = a Centene brand (skip); **Bright Health** = defunct. **Aetna** = CVS (already `cvshealth`). Gig-CSR (Liveops/Arise/NexRep) onboard 1099 CONTRACTORS behind certification/background gates → human-onboarding, not a mass-apply lane.
- **Four "not httpx-collectable" boards — SOLVED + WIRED 2026-09-21 (the wall was the request FINGERPRINT, NOT geo).** A prior recon marked these un-collectable; live re-recon proved a US-datacenter egress (Bright Data US-pinned) got the IDENTICAL rejection as DIRECT, so **US egress does NOT help** — the discriminator is a non-browser TLS/JA3 ClientHello or Cloudflare's JS interstitial. Two are cracked from the plain server IP with **curl_cffi's Chrome TLS impersonation** (added to `backend/requirements.txt`, lazily imported + guarded via `_cffi_session`/`_cffi_get` so a missing wheel never breaks `collect()`); two were never really walled (the earlier recon guessed the wrong endpoint). All four are **COLLECT-FIRST (`auto_status='needs_laptop'`)** — no lane auto-drives them. Tests: `test_geico_*`/`test_afni_*`/`test_cotiviti_*`/`test_progressive_*` in `test_mass_hiring.py` (network-free).
  - **GEICO** (`fetch_geico` → `_fetch_workday`): NOT Incapsula-walled after all — the `careers.geico.com` React SPA sits behind Incapsula, but "See Open Jobs" points at an **UNPROTECTED Workday tenant** `geico.wd1.myworkdayjobs.com/geico/External` reachable by **PLAIN httpx** (the "Phenom `/widgets`" guess was wrong). `_fetch_workday(..., search_texts=("","remote"), title_remote=True, us_confirmed=True)`. **HONEST live yield: 285 postings, ~28 remote — but every remote role is senior/professional (Counsel / Sr Engineer / Sales MANAGER / Investigator) → 0 remote-US ENTRY today** (GEICO's entry CSR/claims reps are ONSITE). Reachable + future-proof (seasonal remote CSR ramps auto-captured). Apply = Workday create-account lane after a per-tenant verify.
...
- **Sutherland / SmartRecruiters** (`strategies/smartrecruiters.py`, cron `mass_hiring_apply_sr_cron`) — full-auto to a real
  ...
  **The strategy is COMPANY-AGNOSTIC** (shadow-DOM `<oc-button>` wizard, no Sutherland-specific code), and `sr_job_ids()`
  selects jobs by the `jobs.smartrecruiters.com` **apply_url HOST, NOT `source`** — so ANY new SmartRecruiters employer
  (e.g. **Wayfair**, added 2026-09-21) is auto-picked up by this ONE cron with **NO cron scope-widening + NO new cron line**;
  `job_is_staffable` still vetoes any language/license gate. Confirmed by code inspection for Wayfair; the live SR fill was NOT
  drive-verified for Wayfair specifically because the board has 0 remote-US postings right now (nothing to drive) — the
  identical Sutherland flow is the proof of the shared strategy.
- **Centene / Workday CxS** (`tools/workday_recon.py` `drive_apply`, cron `mass_hiring_apply_workday_cron --tenant centene`) —
  ...
  **HEALTHCARE-PAYER TENANTS collected + strategy-routed 2026-09-21 (Elevance/Anthem `elevance`, Highmark `highmark`, Sagility
  `sagility`) — REGISTER-CAPTCHA PROBE PENDING, in `_BLOCKED` NOT `_LIVE_TENANTS`.** Collectors `fetch_elevance`/`fetch_highmark`/
  `fetch_sagility` (see the connector list for the `remote_location_facet`+`title_remote` mechanism) feed `mass_hiring_jobs`; the
  hosts are in `mass_hiring_apply.SUPPORTED_HOSTS` + `strategies/workday.py::_MASSHIRING_HOST_RE` so a probe routes to the
  `WorkdayMassHiringStrategy` create-account wizard (NOT the stock strategy). LIVE COLLECTOR YIELD 2026-09-21 (remote+entry):
  **Elevance 1 · Highmark 6 · Sagility 12** (Elevance's ANT site is ~entirely office-based — the "272 postings" are office/hybrid,
  only ~1 truly remote). To PROMOTE: on a QUIET `:98`, `DISPLAY=:98 WORKDAY_ADVANCE=1 WORKDAY_DEBUG_SHOTS=1 sg mail -c 'PYTHONPATH=.
