...
- **NOT httpx-collectable — recon 2026-09-21, documented so they aren't re-chased** (all need a headful pass / US-residential IP; deliberately NOT added as connectors — a guessed endpoint would collect 0 forever): **Progressive** (Workday CxS `progressive.wd5.myworkdayjobs.com` — the CxS endpoint returns an empty-body **HTTP_422** edge-WAF rejection on every site-path guess, careers domains 403; a wrong site on a REACHABLE tenant gives a clean 404 with `Job_Posting_Site_ID=…`, so this 422 is a datacenter-IP WAF block NOT a bad site → the `<SITE>` slug is unresolvable via httpx, needs a headful read; then it'd be a `_fetch_workday` one-liner). **Cotiviti** (iCIMS `careers-cotiviti.icims.com` — every `/jobs/search` GET returns a 405 **"Human Verification"** challenge to the datacenter IP; the iCIMS APPLY strategy exists (TP lane) but collection is bot-walled). **GEICO** (the research "Phenom `/widgets`" guess is WRONG — `careers.geico.com` is a custom React SPA behind **Incapsula/Imperva** (`/_Incapsula_Resource`), `/widgets` 404s; NO Phenom endpoint, and Phenom apply isn't a built strategy anyway → needs headful RE). **Afni** (ADP Recruiting — the old `recruiting.adp.com/srccar?c=1052341` portal 302-redirects to a NEW Angular SPA `myjobs.adp.com/afniexternalcareers`; its job API is `{config.myadpUrl}/myadp_prefix/mycareer/public/staffing/v1/job-requisitions/search`, a runtime-config-host + `withCredentials`-gated endpoint not reachable by blind httpx — every guess returns the SPA shell / 302; ADP is a NEW ATS with no existing apply strategy → needs headful capture + a new driver).
