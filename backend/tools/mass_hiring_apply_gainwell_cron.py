"""Cron: real-submit to every auto-applyable Gainwell (SuccessFactors) job once per run.

The Gainwell analogue of the Foundever lane (same SuccessFactors careersection family, careersection
host career41.sapsf.com / company gainwellte). The careersection has NO submit captcha, so each
application is driven HEADLESS by `backend.tools.gainwell_recon`
(`GAINWELL_HEADLESS=1 GAINWELL_ADVANCE=1`) which fills the whole single-page SuccessFactors form and
lands a real account/"application received" email in the persona's @takhet.com box. gainwell_recon
isolates its Chromium profile dir per pid, so runs can go in PARALLEL (`--workers`).

    python backend/tools/mass_hiring_apply_gainwell_cron.py             # 1 application per Gainwell job
    python backend/tools/mass_hiring_apply_gainwell_cron.py --only 1426420500
    python backend/tools/mass_hiring_apply_gainwell_cron.py --workers 2 --skip-confirmed

Run under `sg mail` (gainwell_recon needs the mail group for mailbox provisioning + the Maildir
...
logger = logging.getLogger("gainwell_apply_cron")

REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, REPO)

LOCK_PATH = os.path.join(REPO, "logs", "gainwell_apply.lock")
...
_CONFIRM_SUBJECT_RE = re.compile(
    ...
    r"application received|application has been received|career portal", re.I)
...
def _is_confirmation(from_hdr: str, subject: str) -> bool:
    """True iff this mail is the Gainwell/SuccessFactors 'application received' confirmation: from a
    successfactors/sapsf/gainwell sender, or a matching 'thank you for applying' subject."""
    frm = (from_hdr or "").lower()
    if any(h in frm for h in ("successfactors", "sapsf", "gainwell")):
        return True
...
def apply_one(jobid: int, keep: int) -> dict:
    """Drive ONE Gainwell application via gainwell_recon (headless, fresh persona). Never raises."""
    env = dict(os.environ)
    env.update({"GAINWELL_HEADLESS": "1", "GAINWELL_ADVANCE": "1"})
    started = time.time()
    out = ""
    error = None
    try:
        proc = subprocess.run(
            [sys.executable, "-m", "backend.tools.gainwell_recon",
        ...
        error = "timeout"
        try:
            subprocess.run(["pkill", "-f", f"gainwell_recon --job {jobid}"], timeout=20)
        except Exception:
            pass
    ...
    # the on-page "Your Application has been sent" (SUBMITTED) is Gainwell's own application ack; the
...
def _confirmed_jobids_in_log() -> set:
    ids = set()
    try:
        with open(os.path.join(REPO, "logs", "gainwell_apply.log")) as f:
            for line in f:
...
def main() -> None:
    ...
    ap.add_argument("--limit", type=int, default=0, help="apply to only the first N Gainwell jobs")
    ...
    ap.add_argument("--rounds", type=int, default=1, help="applications per Gainwell job this run")
    ...
    ap.add_argument("--skip-confirmed", action="store_true",
                    help="skip jobids already confirmed=True in gainwell_apply.log")
    args = ap.parse_args()
    ...
    try:
        ...
        logger.info("a previous Gainwell apply run is still going — exiting")
        return

    if args.only:
        ids = [args.only]
    else:
        from backend.tools.gainwell_recon import gainwell_job_ids
        ...
        if args.limit and args.limit > 0:
            ids = ids[:args.limit]
    if not ids:
        logger.info("no auto-applyable Gainwell jobs on the board")
        return
    ...
    logger.info("applying to %d Gainwell jobs x %d round(s) = %d applications (workers=%d, pay-ordered, open-only)",
                len(ids), args.rounds, len(batch), workers)
    if not batch:
        logger.info("every Gainwell job already reached interview/offer — nothing to apply")
        return
    ...
    logger.info("gainwell apply run done: %d jobs, submitted=%d, confirmed=%d",
